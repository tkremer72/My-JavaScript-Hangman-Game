...
var _game_status = require("./game_status");

var status = _interopRequireWildcard(_game_status);
...
function displayStatus(word, letters) {
  return `<div>${getMessage(word, letters)}</div>`;
}

},{"./game_status":3}],3:[function(require,module,exports){
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.lettersRemaining = lettersRemaining;
exports.isGameWon = isGameWon;
exports.isGameOver = isGameOver;
exports.isStillPlaying = isStillPlaying;
//Set the wrong guesses
const MAX_WRONG_LETTERS = 10; //Find all the letters

function lettersRemaining(word, letters) {
...
function isGameWon(word, letters) {
  return !word.split("").find(letter => !letters.includes(letter));
} //If no letters and game not won, game over


function isGameOver(word, letters) {
  return !lettersRemaining(word, letters) && !isGameWon(word, letters);
} //Still in play


function isStillPlaying(word, letters) {
  return lettersRemaining(word, letters) && !isGameOver(word, letters) && !isGameWon(word, letters);
}
...
var _words_list = require("./words_list");

var _words_list2 = _interopRequireDefault(_words_list);
...
var _keyboard_layout = require("./keyboard_layout");

var _keyboard_layout2 = _interopRequireDefault(_keyboard_layout);

var _game_display_status = require("./game_display_status");

var _game_display_status2 = _interopRequireDefault(_game_display_status);

var _new_game = require("./new_game");
...
function drawGame(word, letters) {
  document.querySelector('#display-status').innerHTML = (0, _game_display_status2.default)(word, letters);
  document.querySelector('#letter-slots').innerHTML = (0, _letter_slots2.default)(word, letters);
  document.querySelector('#keyboard').innerHTML = (0, _keyboard_layout2.default)(letters);
}

(0, _words_list2.default)(word => {
  const letters = [];
  document.addEventListener('click', event => {
    if ((0, _game_status.isStillPlaying)(word, letters) && event.target.tagName === 'BUTTON') {
      letters.push(event.target.dataset.character);
      drawGame(word, letters);
    }
  });
  drawGame(word, letters);
  (0, _new_game2.default)();
});

},{"./game_display_status":2,"./game_status":3,"./keyboard_layout":5,"./letter_slots":6,"./new_game":7,"./words_list":9,"regenerator-runtime/runtime":1}],5:[function(require,module,exports){
"use strict";
...
//Store the letters
const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split(""); //Divide the letters into three rows

const row1 = alphabet.slice(0, 10);
const row2 = alphabet.slice(10, 19);
const row3 = alphabet.slice(19, 26); //Show or hide the buttons
...
function letterSlots(word, letters) {
...
exports.default = newHttpRequest;

//Create an http request 
function newHttpRequest(method, url, data) {
  return fetch(url, {
    method: method,
    body: data
  }).then(response => {
...
var _send_http_request = require("./send_http_request");

var _send_http_request2 = _interopRequireDefault(_send_http_request);
...
async function getWords(cb) {
  try {
    cb(await (0, _send_http_request2.default)('GET', 'https://random-word-api.herokuapp.com/word?number=50'));
  } catch (error) {
    alert(error.message);
  }
...
async function getRandomWord(cb) {
  try {
